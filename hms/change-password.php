...
if(isset($_POST['submit'])) {
    $sql=mysqli_query($connect,"SELECT password FROM  users where password='".md5($_POST['cpass'])."' && email='".$_SESSION['login']."'");
    $num=mysqli_fetch_array($sql);
    if($num>0) {
        $con=mysqli_query($connect,"update users set password='".md5($_POST['npass'])."', updationDate='$currentTime' where email='".$_SESSION['login']."'");
